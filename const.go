package onectechcommon

const (
	RPHOST                            = "rphost"
	RMNGR                             = "rmngr"
	CONTEXT                           = "Context"
	EXCP                              = "EXCP"
	CALL                              = "CALL"
	PROCESS                           = "process"
	P_PROCESS_NAME                    = "p:processName"
	EXCEPTION                         = "Exception"
	DESCR                             = "Descr"
	DATABASE_EXCEPTION                = "DataBaseException"
	DB_LOCK_MSSQL                     = "Lock request time out period exceeded"
	DB_LOCK_POSTGRS                   = "canceling statement due to lock timeout"
	DB_DEADLOCK_MSSQL                 = "was deadlocked on lock resources with another process and has been chosen as the deadlock victim"
	CANNOT_INSERT_DUPLICATE_KEY_MSSQL = "Cannot insert duplicate key"
	CPU_TIME                          = "CpuTime"
	MEMORY_PEAK                       = "MemoryPeak"
	IN_BYTES                          = "InBytes"
	OUT_BYTES                         = "OutBytes"
	SERVER_JOB_EXECUTOR_CONTEXT       = "ServerJobExecutorContext"
	TTIMEOUT                          = "TTIMEOUT"
	TDEADLOCK                         = "TDEADLOCK"
	TLOCK                             = "TLOCK"
	DEBUG_CONTROL_CENTER              = "DebugControlCenter"
	ADMIN_PROCESS                     = "##AdminProcess##"
	BAD_ALLOC                         = "bad_alloc"
	TRANSACTION_ALREADY_HAS_ERRORS_RU = "В данной транзакции уже происходили ошибки"
	FORM_UNAVAILABLE_FOR_USE_RU       = "Форма недоступна для использования"
	DBMSSQL                           = "DBMSSQL"
	DBPOSTGRS                         = "DBPOSTGRS"
	SQL                               = "Sql"
	ROWS                              = "Rows"
	ROWS_AFFECTED                     = "RowsAffected"
	COMPUTER_NAME                     = "t:computerName"
	OS_THREAD                         = "OSThread"
	T_CONNECT_ID                      = "t:connectID"
	WAIT_CONNECTION                   = "WaitConnections"
	REGIONS                           = "Regions"
	LOCKS                             = "Locks"
	FUNC                              = "Func"
	SDBL                              = "SDBL"
	BEGIN_TRANSACTION                 = "BeginTransaction"
	EXCLUSIVE                         = "Exclusive"
	SHARED                            = "Shared"
	MODULE                            = "Module"
	METHOD                            = "Method"
)
